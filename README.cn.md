# gptscript

[English version](./README.md)

> ⚠️ 此项目已归档（archived）。

Build AI assistants that interact with your systems

[![x-cmd/install — gptscript Code Quality Monitoring Repo Card](https://x-cmd.com/repo-card/gptscript.svg?lang=zh)](https://x-cmd.com/install/gptscript)

## 安装

```sh
x install gptscript
```

## 代码洞察

合计: **25,040** 行代码（覆盖前 5 种语言、共 **188** 个文件）。

| 语言 | 代码 | 注释 | 空行 | 文件数 |
|------|-----:|-----:|-----:|------:|
| Go | 20,035 | 520 | 3,224 | 158 |
| Json | 3,721 | 0 | 0 | 19 |
| Yaml | 384 | 1 | 11 | 4 |
| JavaScript | 331 | 46 | 40 | 6 |
| Sh | 218 | 50 | 37 | 1 |

## 源代码

- **上游仓库**: <https://github.com/gptscript-ai/gptscript>
- **官网**: <https://gptscript.ai>
- **许可证**: Apache-2.0

## 发布

- **最新版本**: `v0.9.9` (2026-04-10)
- **最近提交**: 2026-05-22
- **Release 含资产**: 6 个

## 流行度

- **Star**: 3,279 · **Fork**: 296 · **开放 issue**: 259 · **贡献者**: 30

## 累计统计

- **发布数**: 59 · **已合并 PR**: 641 · **开放 PR**: 10 · **已关闭 issue**: 184 · **开放 issue**: 75 · **提交数**: 1123

## 最近活动

| 时间窗口 | 起始 | 发布 | 已合并 PR | 开放 PR | 已关闭 issue | 开放 issue | 提交 |
|---|---|---:|---:|---:|---:|---:|---:|
| 30d | 2026-08-24 | 0 | 0 | 0 | 0 | 0 | 0 |
| last60d | 2026-07-25 | 0 | 0 | 0 | 0 | 0 | 0 |
| 90d | 2026-06-25 | 0 | 0 | 0 | 0 | 0 | 0 |
| last180d | 2026-03-27 | 1 | 5 | 1 | 0 | 0 | 5 |
| 360d | 2025-09-28 | 4 | 12 | 2 | 0 | 0 | 13 |
| last720d | 2024-10-03 | 4 | 115 | 4 | 6 | 12 | 128 |

## Release 资产

| 资产 | 大小 | 目标平台 |
|------|-----:|----------|
| [checksums.txt](https://github.com/gptscript-ai/gptscript/releases/download/v0.9.9/checksums.txt) | 512 B | `other` |
| [gptscript-v0.9.9-linux-amd64.tar.gz](https://github.com/gptscript-ai/gptscript/releases/download/v0.9.9/gptscript-v0.9.9-linux-amd64.tar.gz) | 16.0 MiB | `native/linux/x64` |
| [gptscript-v0.9.9-linux-arm64.tar.gz](https://github.com/gptscript-ai/gptscript/releases/download/v0.9.9/gptscript-v0.9.9-linux-arm64.tar.gz) | 14.8 MiB | `native/linux/arm64` |
| [gptscript-v0.9.9-macOS-universal.tar.gz](https://github.com/gptscript-ai/gptscript/releases/download/v0.9.9/gptscript-v0.9.9-macOS-universal.tar.gz) | 31.4 MiB | `native/darwin/x64` |
| [gptscript-v0.9.9-windows-amd64.zip](https://github.com/gptscript-ai/gptscript/releases/download/v0.9.9/gptscript-v0.9.9-windows-amd64.zip) | 16.3 MiB | `native/win/x64` |
| [gptscript-v0.9.9-windows-arm64.zip](https://github.com/gptscript-ai/gptscript/releases/download/v0.9.9/gptscript-v0.9.9-windows-arm64.zip) | 14.9 MiB | `native/win/arm64` |

## 改进这些数据

gptscript 的安装元数据由 [x-cmd/install](https://github.com/x-cmd/install) 索引维护——这是一份由 x-cmd 在安装时读取的精选 YAML 包列表。如果 `gptscript` 缺失、过期，或安装行为有问题，欢迎在该 repo 提 issue 或 PR：

- **提交 issue**: <https://github.com/x-cmd/install/issues/new>
- **编辑包条目**: <https://github.com/x-cmd/install/edit/main/gptscript.yml>（或索引实际使用的路径）

本页面的数据（card / loc / scorecard / release）由 [x-cmd-install-action](https://github.com/x-cmd-install/x-cmd-install-action) 自动采集，每日重新生成。**安装行为**（版本选择、平台差异、依赖处理）的改进应提交到上游索引。

_数据快照: `data/card/260923.yml` · 2026-09-23T05:03:17Z._
